...
methods.createNotification = async data => {
  try {
    ...
    let code = utils.errorCode("log_CREATE_ERROR"),
      message = utils.errorMessage(code);
...
methods.getAnalytics = async data => {
  try {
    ...
    let [
      ...
      site_records
    ] = await getAggregateNotificationRec(search);
    ...
    Notification.aggregate([
      {
        $facet: {
          ...
          groupByDay: [
            ...
            {
              $group: {
                _id: "$notification_view",
                notification_view: { $sum: 1 }
              }
            }
          ],
