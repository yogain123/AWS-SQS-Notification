...
(exports.createNotification = joi
  .array()
  .min(1)
  .items(joi.object(notificationScheme))
  .required()),
  //To validate query in request body
...
exports.getErrorMessage = function(err) {
  ...
  if (err.details[0].path.length == 1) return err.details[0].message;
  else {
    var path = err.details[0].path.join(".");
    if (path.trim().length > 0) path = " at path " + path;
